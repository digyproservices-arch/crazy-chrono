  ...
  const loadTournamentData = async () => {
    try {
      ...
      try {
          ...
          if (d.success && d.students) {
            ...
            try {
              sessionStorage.setItem(CACHE_KEY_TOURNAMENT, JSON.stringify({
                ...
                performance: d.performance, tourStatus: d.tourStatus, _ts: Date.now()
              }));
            } catch {}
            loaded = true;
          }
        ...
        console.warn('[TrainingArena] ⚠️ setup-data indisponible, fallback séquentiel:', e.message);
      }
      
      // Fallback : anciens endpoints séquentiels
      if (!loaded) {
        console.log('[TrainingArena] 🔄 Fallback: chargement séquentiel...');
        ...
        console.log('[TrainingArena] ✅ Fallback terminé — Students:', studentsData.students?.length, 'Groups:', groupsData.groups?.length);
      }
    } catch (error) {
