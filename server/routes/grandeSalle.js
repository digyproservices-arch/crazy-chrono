  ...
  try {
    ...
    if (tournament.session_id) {
      const { data: roundRows } = await supabaseAdmin
        .from('match_rounds')
        .select('round_number, zones, good_pair_type, good_pair_theme, good_pair_level, good_pair_content, winner_display_name, winner_time_ms, pairs_found, boards, errors')
